...
int fitnessfunc()
{
    ...
    {
        int x=0;
        #pragma omp parallel num_threads(attributeCount)
        	#pragma omp atomic
        	x+=costmat[(population[individual][omp_get_thread_num()])][population[individual][(omp_get_thread_num()+1)%attributeCount]];
        
        /*for(attribute=0;attribute<attributeCount;attribute+=threads)
        {
        	#pragma omp atomic
            x+=costmat[(population[individual][attribute])][population[individual][(attribute+1)%attributeCount]];
...
int fitnessfrequencyCalc()
{
    ...
    for(individual=0;individual<populationsize;individual++)
    {
       ...
       /* #pragma omp parallel num_threads(populationsize)
        {
        	if(fitness[individual]==fitness[omp_get_thread_num()] && omp_get_thread_num()<populationsize)
        	{
        		#pragma omp atomic
...
int removeindividual(int individualCurrent)
{
    ...
    if(individualCurrent<populationsize)
    {
        ...
        {
        	//#pragma omp parallel num_threads(attributeCount)
        	//	population[i][omp_get_thread_num()]=population[i+1][omp_get_thread_num()];
...
int main()
{
    ...
    for(int iteration=0;iteration<iterationCount;iteration++)
    {
        ...
        printf("Processed Population Parallel %d\n",populationsize);
        printf("=====================================================================\n");
        for(individual=0;individual<populationsize;individual++)
        {
            for(attribute=0;attribute<attributeCount;attribute++)
